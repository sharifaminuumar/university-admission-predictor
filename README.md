...
The app is organised into six views, each with its own URL hash so any of them can be linked to directly. Navigation sits right-aligned in the header on desktop and collapses into a slide-in drawer below 1024px.

### 🏠 Home (`#home`)
The default landing view: hero, live metrics banner, an infinite marquee of every supported institution with colour-coded crest monograms, a three-step "how it works" walkthrough, the value-proposition grid (data honesty, D7 diploma pathways, private & free), and a data-verification card.
...
### Footer
A three-column footer covering the platform mission and disclaimer with quick links, the verification and benchmarking note (official university portals, prospectuses and gazettes), and creator links to GitHub and LinkedIn.
...
| `/api/universities` | GET | Summary of every institution — region, programme and diploma counts, and published cut-off range. Powers the home metrics, marquee and directory. |
